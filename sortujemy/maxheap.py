...
for i in range (12):
    list = kopiec()
    M[i] = random.randint(1000000, 5000000)
    print(M)
    for j in range (M[i]):
...
#slope, intercept, r_value, p_value, std_err = stats.linregress(TT0, M)
slope, intercept, r_value, p_value, std_err = stats.linregress(log2, timeArr)

a = slope
print ("a =", a)
b = intercept

#plt.plot(TT0, M, '.', label = 'Original data')
plt.plot(log2, timeArr, '.', label = 'Original data')
#plt.plot(TT0, a*TT0 + b, '-', label = 'Fitted line')
plt.plot(log2, a*log2 + b, '-', label = 'Fitted line')
plt.xlabel("log2(M)")
plt.ylabel("log2(T/(T0*log2(M)))")
plt.legend()
plt.show()
